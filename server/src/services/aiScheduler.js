...
async function runNightlyScan() {
  console.log('[truein] AI Time Guard nightly scan starting…');
  try {
    ...
    console.log(`[truein] AI Time Guard scan complete: ${results.length} org(s), ${totalUpserted} anomalies upserted`);
  } catch (err) {
    console.error('[truein] AI Time Guard nightly scan failed:', err.message);
  }
}
...
function start() {
  ...
  cron.schedule('0 2 * * *', () => {
    runNightlyScan().catch((err) => console.error('[truein] AI scheduler tick failed:', err.message));
  });
  console.log('[truein] AI Time Guard scheduler started (daily 02:00)');
}
